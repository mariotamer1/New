(function () {
  ...
  var controls = document.querySelector(".sights-controls");
  var prevBtn = document.querySelector(".sight-prev");
  var nextBtn = document.querySelector(".sight-next");
  var originalCards = track ? Array.prototype.slice.call(track.children) : [];

  if (!section || !track || originalCards.length === 0) return;

  var targetMouseX = 0, targetMouseY = 0, mouseX = 0, mouseY = 0;
  var targetScroll = 0, smoothScroll = 0, initialized = false, rafPending = false;
  var sightCards = [];
  ...
  function update() {
    ...
    var mxOut = reduceMotion.matches ? 0 : mouseX;
    var myOut = reduceMotion.matches ? 0 : mouseY;
  ...
  function updateSightSlider() {
    ...
    sightCards.forEach(function (card, i) {
